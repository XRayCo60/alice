...
static class WarEngineV2Core
{
    // Disabled in the bot integration; the standalone v2 entry points can opt in later.
    const bool StandalonePersistenceEnabled = false;
    // ───────────────────────────── ثوابت میدان ─────────────────────────────
    ...
    {
        // Program.cs owns durable battle history and settlement. Keeping persistence here
        // would insert every battle twice when this v2 core is used through the adapter.
        if (!StandalonePersistenceEnabled) return;
        try
        {
