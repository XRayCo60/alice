...
static class WarEngine
{
    ...
    static void BrainStaticLine(Force me, Force foe, Field field, float depth, int tick, BattleLog log, ref XorRng rng)
    {
        ...
        for (int i = 0; i < me.N; i++)
        {
            ...
            {
                //  خط اول فقط ذخیره نیست که جابه‌جا شود. یگانی که سکتور خودش
                //  آرام است و صدای درگیری از چند کیلومتر آن‌طرف‌تر می‌آید،
                //  در جای خود نمی‌ماند — در امتداد جبهه به سمت فشار می‌لغزد.
                //  بدون این، ۶۰٪ نیروی مدافع تا آخر نبرد در سکتور خالی خودش
                //  می‌ایستد و مهاجم با تمرکز روی یک نقطه، برتری محلی نجومی
                //  می‌گیرد در حالی که نسبت کلی قوا برابر است.
                int mySec = Math.Clamp((int)(g.X / SECTOR_KM), 0, SECTORS - 1);
                float myThreat = me.ThreatMap[mySec];
                float gap = Math.Abs(g.X - hotX);
                bool quietHere = hv > 0f && myThreat < hv * 0.35f;
                bool reachable = gap > SECTOR_KM * 0.75f && gap < LATERAL_REACH;
                if (quietHere && reachable)
                {
                    g.Posture = P_ADVANCE;
                    g.TgtX = Math.Clamp(hotX + g.Lane * 4f, 1f, FRONT_KM - 1);
                    g.TgtY = g.Y;              // در امتداد جبهه، نه به جلو
                }
    ...
    //  بیشترین فاصله‌ای که یک یگان خط اول حاضر است در امتداد جبهه بلغزد تا
    //  به نقطه‌ی فشار برسد. دورتر از این، رها کردن سکتور خودی بی‌احتیاطی است.
    const float LATERAL_REACH = 12f;
    ...
    //
    //  شیب قبلاً ۶.۲ بود و منحنی تقریباً پلکانی می‌شد: نسبت ۰.۷ یعنی ۱۳٪ و
    //  نسبت ۱.۳ یعنی ۸۷٪. یعنی یک اختلاف کوچک در نفوذ، نتیجه‌ی نبرد را
    //  کاملاً وارونه می‌کرد و تقابل مدل‌ها به «برنده‌ی مطلق / بازنده‌ی مطلق»
    //  تبدیل می‌شد. اندازه‌گیری نشان داد نسبت کشتار بین بدترین و بهترین
    //  تقابل به ۹۲ برابر می‌رسید.
    //
    //  واقعیت این‌طور نیست:
    //   • گلوله‌ای که از سینه رد نمی‌شود، شنی و چرخ و اپتیک و لوله را می‌زند؛
    //     تانک از کار می‌افتد حتی اگر سوراخ نشود. پس کفِ ۶٪ منطقی است.
    //   • گلوله‌ای که به‌راحتی نفوذ می‌کند هم همیشه خدمه یا مهمات را نمی‌زند؛
    //     سقفِ ۹۴٪ یعنی هیچ برخوردی «کشتار تضمینی» نیست.
    //  شیب نصف شد تا برتری زره مزیت بدهد، نه مصونیت.
    static float PenChance(float penMm, float armorMm)
    {
        float ratio = penMm / MathF.Max(1f, armorMm);
        float core = 1f / (1f + MathF.Exp(-(ratio - 1f) * 3.1f));
        return 0.06f + 0.88f * core;
    }

    // آهنگ آتش مؤثر در یک درگیری شش‌دقیقه‌ای.
    //  آهنگ آتش کاتالوگی (چرخه‌ی بیشینه) با آهنگ پایدار در نبرد یکی نیست:
    //  خدمه باید هدف بگیرد، اصابت را ببیند، تصحیح کند و دوباره بیابد. این
    //  سربارِ ثابت، فاصله‌ی بین توپ تندآتش و توپ کندآتش را فشرده می‌کند.
    //
    //  قبلاً RoF مستقیم و خطی ضرب می‌شد، پس M2 با ۲۰ گلوله در دقیقه چهار برابر
    //  T-28 با ۵ گلوله شلیک می‌کرد و عملاً هیچ مزیتی از توپ ۷۶ میلی‌متری باقی
    //  نمی‌ماند. با توان ۰.۶ حول مرجع ۱۰ گلوله در دقیقه، نسبت چهار برابری به
    //  حدود ۲.۳ برابر می‌رسد — همچنان مزیت روشن، ولی نه حذف کامل حریف.
    static float EffectiveRoF(float rofPerMin)
        => 10f * MathF.Pow(MathF.Max(0.5f, rofPerMin) / 10f, 0.6f);
            ...
            if (u.Type == 1)
            {
                ...
                else
                {
                    ...
                    if (u.CAmmo >= 1f)
                    {
                        ...
                        //  شعاع کشندگی با ریشه‌ی وزن ماده‌ی منفجره بالا می‌رود.
                        //   توان ۰.۳۳۳ بیش از حد صاف بود: گلوله‌ی ۶۲۱ گرمی T-28 فقط
                        //   ۲.۵ برابر گلوله‌ی ۳۹ گرمی M2 اثر می‌گرفت، در حالی که پانزده
